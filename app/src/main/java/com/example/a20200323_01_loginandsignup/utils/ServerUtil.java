...
//    서버 호스트 주소를 편하게 가져다 쓰려고 변수로 저장
    private static final String BASE_URL = "http://http://192.168.0.236:5000";

//    로그인 요청 기능 메소드
//    파라미터의 기본구조 : 어떤화면에서? 어떤응답처리를 할지? 변수로.
//    파라미터 추가 : 서버로 전달할때 필요한 데이터들을 변수로.
    public static void postRequestLogi(Context context, String id, String pw, JsonResponseHandler handler){
...
//        서버로 들고갈 파라미터를 담아줘야함.
        FormBody fromData = new FormBody.Builder()
                .add("login_id", id)
                .add("password", pw)
                .build();
